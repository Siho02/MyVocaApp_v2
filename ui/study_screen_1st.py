...
class StudyScreen(QWidget):
    ...
    def init_ui(self):
        ...
        self.submit_button.clicked.connect(self.check_subjective_ansewr)
    ...
    def show_objective_eng_to_kor(self):
        ...
        for w in self.all_words:
            if w['word'] != self.current_word['word']:
                all_meanings.extend(w.get("meaning", []))
    ...
    def show_subjective_question(self):
        ...
        if self.mode == 'eng_to_kor':
            ...
            self.label_question.setText(f"'{question}'와 같은 의미를 가진 영어 단어는?")
    ...
    def check_subjective_ansewr(self):
